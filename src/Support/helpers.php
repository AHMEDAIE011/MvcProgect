...
if (!function_exists('viewAdmin')) {
    function viewAdmin($view, $params = [])
    {
        echo View::makeAdmin($view, $params);
    }
}
...
if (!function_exists('UserGetName')) {
    function UserGetName()
    {
        print_r(Arr::flatten(app()->session->get('user')[0])[5]);
    }
}

if (!function_exists('UserGet')) {
    function UserGet($id)
    {
        $data = Arr::flatten(app()->session->get('user')[0])[$id];
